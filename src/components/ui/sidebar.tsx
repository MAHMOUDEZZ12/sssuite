...
>(
  (
    ...
    return (
      <div
        ref={ref}
        className="group peer hidden md:flex flex-col"
        data-state={state}
...
>(({ className, onClick, ...props }, ref) => {
  ...
  return (
    <Button
      ...
      variant="ghost"
      size="icon"
      className={cn("h-8 w-8", isMobile && "flex", className)}
      onClick={(event) => {
        onClick?.(event)
        toggleSidebar()
      }}
...
>(({ className, ...props }, ref) => {
  return (
    <div
      ref={ref}
      data-sidebar="header"
      className={cn("flex items-center h-16 shrink-0 p-2 border-b", className)}
      {...props}
    />
  )
})
